...
cd ~
git clone https://github.com/rubrettdev/nodejs-pool.git  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
cd /usr/local/src
sudo git clone https://github.com/rubrettdev/UPShare.git
cd UPShare
sudo mkdir build
cd build
sudo cmake ..
sudo make -j$(nproc)
sudo cp ~/nodejs-pool/deployment/upshare.service /lib/systemd/system/
sudo useradd -m upsharedaemon -d /home/upsharedaemon
# BLOCKCHAIN_DOWNLOAD_DIR=$(sudo -u monerodaemon mktemp -d)
# sudo -u upsharedaemon wget --limit-rate=50m -O $BLOCKCHAIN_DOWNLOAD_DIR/blockchain.raw https://downloads.getmonero.org/blockchain.raw
# sudo -u upsharedaemon /usr/local/src/monero/build/release/bin/monero-blockchain-import --input-file $BLOCKCHAIN_DOWNLOAD_DIR/blockchain.raw --batch-size 20000 --database lmdb#fastest --verify off --data-dir /home/monerodaemon/.bitmonero
# sudo -u upsharedaemon rm -rf $BLOCKCHAIN_DOWNLOAD_DIR
sudo systemctl daemon-reload
sudo systemctl enable upshare
sudo systemctl start upshare
...
npm install
npm install -g pm2
openssl req -subj "/C=West/ST=Pool/L=Daemon/O=Mining Pool/CN=west-pool.org" -newkey rsa:2048 -nodes -keyout cert.key -x509 -out cert.pem -days 36500
mkdir ~/pool_db/
...
echo "Your MySQL Root Password is (Just in case you need):"
echo $ROOT_SQL_PASS
# I0qd7VHqvNOVjqoIVrBPavd9zP7gLkug
ROOT_SQL_PASS="CC3p,123"
